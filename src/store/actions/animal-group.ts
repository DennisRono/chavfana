...
export const getAnimalGroups = createAsyncThunk<AnimalGroup[], string, { rejectValue: ErrorResponse }>(
  "animalGroup/getAll",
  async (projectId, { getState, rejectWithValue }) => {
    try {
      const state = getState() as RootState
      const response = await fetch(`${BASE_URL}/api/projects/${projectId}/animal-groups/`, {
        method: "GET",
        headers: {
...
>("animalGroup/create", async ({ projectId, data }, { getState, rejectWithValue }) => {
  try {
    const state = getState() as RootState
    const payload = {
...
export const getAnimalGroupById = createAsyncThunk<
  ...
  { rejectValue: ErrorResponse }
>("animalGroup/getById", async ({ projectId, groupId }, { getState, rejectWithValue }) => {
  try {
    const state = getState() as RootState
    const response = await fetch(`${BASE_URL}/api/projects/${projectId}/animal-groups/${groupId}/`, {
      method: "GET",
      headers: {
        Authorization: `Bearer ${state?.auth?.access_token}`,
      },
...
export const updateAnimalGroup = createAsyncThunk<
  ...
  { rejectValue: ErrorResponse }
>("animalGroup/update", async ({ projectId, groupId, data }, { getState, rejectWithValue }) => {
  try {
    const state = getState() as RootState
    const payload = {
      group_name: data.group_name,
...
export const deleteAnimalGroup = createAsyncThunk<
  ...
  { projectId: string; groupId: string },
  { rejectValue: ErrorResponse }
>("animalGroup/delete", async ({ projectId, groupId }, { getState, rejectWithValue }) => {
  try {
    const state = getState() as RootState
    const response = await fetch(`${BASE_URL}/api/projects/${projectId}/animal-groups/${groupId}/`, {
      method: "DELETE",
